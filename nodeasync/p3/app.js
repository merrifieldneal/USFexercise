$(function () {
    ...
    // 1. all the pokemon
    async function p1() {
        // get all ~1300 pokemon
        let data = await $.getJSON(`${baseURL}/pokemon/?limit=1500`);
        console.log(data);
    }

    // 2. 3 from all the pokemon
    // data for those 3
    async function p2() {
        let data = await $.getJSON(`${baseURL}/pokemon/?limit=1500`);
        let randomPokemonUrls = [];
        // 3 pokemon from the data
        for (let i = 0; i < 3; i++) {
    ...
    // 3. same as 2 but console log name and description of those 3 instead
    async function p3() {
        let Data = await $.getJSON(`${baseURL}/pokemon/?limit=1500`);
        let randomPokemonUrls = [];
        for (let i = 0; i < 3; i++) {
